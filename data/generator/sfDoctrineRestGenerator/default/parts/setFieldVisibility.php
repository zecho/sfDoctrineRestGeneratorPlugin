...
<?php $embedded_relations_fields = $this->configuration->getValue('get.embedded_relations_fields'); ?>
<?php if (count($embedded_relations_fields) > 0): ?>

    $embedded_relations_fields = <?php echo var_export($embedded_relations_fields, true); ?>;

    // relation types
    $relation_types = array(
<?php foreach (array_keys($embedded_relations_fields) as $relation_name): ?>
<?php
	 ...
	 foreach ($embedded_relations_fields as $relation_name => $display_fields)
        {
          if (isset($object[$relation_name]))
          {
            ...
            {
                $object[$relation_name] = array_intersect_key($object[$relation_name], $display_fields);
            }
            else
            {
              ...
              {
		 $related_objects[$j] = array_intersect_key($related_object, $display_fields);
              }
